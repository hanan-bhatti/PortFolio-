...
const githubUrl = `https://github.com/${githubContributions.username}`;
    ...
    () => {
      ...
      cells.forEach((c) => {
        if (c.dataset.level !== "0") {
          const parts = (c.dataset.key as string).split("-").map(Number);
          const col = parts[0] ?? 0;
          const row = parts[1] ?? 0;
      ...
      <div className={styles.inner}>
        ...
        <div className={styles.footer}>
          ...
          <div className={styles.controls}>
            <span className="font-syne text-[11px] uppercase tracking-widest text-text-muted">{totalLabel} in the last year</span>
          </div>
        </div>
      </div>
    </div>
